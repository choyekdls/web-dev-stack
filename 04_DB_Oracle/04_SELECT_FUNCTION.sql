/*
  ...
  >> 함수를 사용할 수 있는 위치 : SELECT, WHEREM ORDER BY, GROUP BY, HAVING
...
*/
SELECT
LENGTH('데이터베이스'), LENGTHB('데이터베이스'), LENGTH('database'), LENGTHB('database')
FROM DUAL; -- DUAL은 가상테이블을 의미함(노의미~)
...
*/
SELECT
INSTR('AABAACAABBAA', 'B', -1, 2), -- 뒤에서부터 2번째에 있는 B (세는 건 무조건 앞에서 부터...)
INSTR('AABAAVAABBAA', 'D')
FROM DUAL;
...
/*
  LPAD(왼) / RPAD(오)(컬럼, 최종적으로 반환할 문자의 길이, 덧붙이고자 하는 문자)
...
/*
  CONCAT(문자열, 문자열)
  - 문자열 두개(딱 두개임 : 더도 말고 덜도 말고 정말 딱 두개!.!) 전달받아 하나로 합친 후 결과 반환
*/
-- 가나다라, 1234를 합치기
...
FROM DUAL;
-- 가나다라, ABCD, 1234를 합피기
-- SELECT )(CONCAT('가나다라', 'ABCD'), '1234') -- 문제 발생. CONCAT은 문자열 두개만 합칠 수 있어
FROM DUAL;
...
-- EMPLOYEE에서 사원들의 이메일 KH.OR.KR을 GMAIL.CO,으로 바꾸기
SELECT 
...
*/
SELECT
ROUND(123.456),--123 (위치 생략시 0: 소수점 날려버리기)
ROUND(123.456, 1), -- 소수점 첫째자리까지
ROUND(123.456, -2)
FROM DUAL;
...
SELECT
TRUNC(123.952),
TRUNC(123.952, 1) -- FLOOR와 다른 점: 소수점 위치를 지정할 수 있다는 점..?
FROM DUAL;
...
FROM DUAL; -- 현재 설정된 화폐 단위에 따라 달라짐
...
-- 날짜 -> 문자
SELECT
TO_CHAR(SYSDATE, 'YYYY'), --년: YYYY, YY, RRRR, RR, YEAR(YEAR은쓰지 맙시다)
TO_CHAR(SYSDATE, 'MM'), --월: MM, MON, MONTH, RM(RM은 로마숫자로 나옴..쓰지 맙시다)
TO_CHAR(SYSDATE, 'DD'), --일: D - 주 단위(일요일-1, ...), DD - 월 단위 ,DDD - 연 단위
TO_CHAR(SYSDATE, 'DAY'), --요일: DY(수), DAY(수요일)
TO_CHAR(SYSDATE, 'PM HH:MI:SS'), --시분초
TO_CHAR(SYSDATE, 'AM HH24:MI:SS') --시분초: 시가 24시 기준으로 나옴 EX) 오후 4시 => 16시
FROM DUAL;
...
SELECT
TO_DATE(20250604164230, 'YYYY-MM-DD HH24:MI:SS')
FROM DUAL;
...
DECODE(JOB_CODE, 'J7', (SALARY*1.1), 'J6', SALARY+(SALARY*1.15), 'J5', SALARY+(SALARY*1.2), SALARY*1.05) AS 인상급여
FROM EMPLOYEE
ORDER BY JOB_CODE, 인상급여 DESC;
-- CASE WHEN 방법 사용
SELECT EMP_NAME,JOB_CODE, SALARY,
CASE WHEN JOB_CODE = 'J7' THEN SALARY*1.1
-- CASE JOB_CODE WHEN 'J7'으로 써도댐, 이런 경우에만
...
CASE WHEN SALARY >= 5000000 THEN '1등급'
     ...
     -- WHEN SALARY >= 3500000 THEN '2등급' 으로만 적어도 됨 500이하는 위 조건 때문에 알아서 걸러짐~!
...
/*
  그룹 함수 -> 결과값이 1개다! (단일행 함수와 함께 사용할 수 없음)
...
/*
  AVG
  - 해당 컬럼값들의 평균값
  - 모든 그룹함수는 NULL값을 자동으로 제외하기 때문에 NVL 함수랑 함께 사용할 것을 권장합니도
*/
-- USER_INFO에서 평균 나이
SELECT
...
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE)
GROUP BY CUBE(JOB_NAME); -- 컬럼 하나일 때는 ROLLUP과 차이 없음
