/*
  ...
  : 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQUENCE), 인덱스(INDEX), 패키지(PACKAGE), 트리거(TRIGGER), 프로시저(PROCEDURE), 함수(FUNCTION), 동의어(SYNONYM), 사용자(USER)
*/
...
/*
  ...
  테이블 생성
  테이블이란? 
  : 행(ROW)과 열(COLUMN)로 구성되는 가장 기본적인 데이터베이스
    ...
    * 자료형
    1. 문자(CHAR / VARCHAR2) - 반드시 크기를 지정해야 함
       - CHAR : 최대 2000BYTE까지 저장 가능
                고정 길이 (아무리 작은 값이 들어와도 처음 할당된 크기 그대로 사용)
...
/*
  ...
  - 제약조건은 데이터 무결성을 보장을 목적으로 한다. (데이터의 정확성과 일관성을 유지시키는 것)
...
INSERT INTO MEM_NOTNULL(MEM_NO, MEM_ID, MEM_PWD, MEM_NAME) VALUES(2, 'user02', 'pass02', '이진용', SYSDATE);
...
INSERT INTO MEM_PRI2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, PHONE, EMAIL, MEM_DATE)VALUES(MEM_SEQ.NEXTVAL, 'user01', 'pass01', '윤의진', '남', '010-1111-2222', 'AAA@naver.com', '25/06/05');
INSERT INTO MEM_PRI2(MEM_ID, MEM_PWD, MEM_NAME, MEM_DATE, GENDER) VALUES(MEM_SEQ.NEXTVAL, 'user02', 'pass02', '이진용', SYSDATE, '남');
INSERT INTO MEM_PRI2(MEM_ID, MEM_PWD, MEM_NAME, GENDER, MEM_DATE, EMAIL) VALUES(MEM_SEQ.NEXTVAL, 'user03', 'pass03', '곽병현', '남', SYSDATE, 'bbb@google.com');
SELECT * FROM MEM_PRI;
...
/*
  ...
  --> FOREIFN KEY를 건다는 것 = 다른 테이블을 참조한다고 표현
  --> 주로 FOREIFN KEY 제약조건에 의해 테이블 간의 관계가 형성됨
*/
...
-- 부모테이블(MEM_GRADE)에서 테이터 값을 삭제할 경우 문제 발생!
...
/*
  ...
  * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는 자식테이블의 값을 어떻게 처리할 건지 설정하는 것
...
DELETE FROM MEM_GRADE WHERE GRADE_CODE = ;
...
VALUES('누구도 대신 살아 주지 않는다', '브라이언 트레이시', 4);
INSERT INTO BOOK (BK_TITLE, BK_AUTHOR, PUB_NO)
VALUES('게으르게 살지만 부자는 되고 싶어', '예프리', 3); -- 왜 안넣어지지..?
INSERT INTO BOOK (BK_TITLE, BK_AUTHOR, PUB_NO)
VALUES('왜 항상 아가리로만 할까?', '이창현', 3);
...
INSERT INTO USER_INFO(NAME) VALUES(USER_INFO_SEQ.NEXTVAL, '테스트')

/*
  1-3. 컬럼 삭제 (DROP)
  DROP COLUMN 컬럼명;
  - 데이터 값이 있어도 같이 삭제됨 (삭제딘 컬럼 복구는 불가능)
*/
...
*/
ALTER TABLE USER_INFO RENAME COLUMN CONTACT TOPHONE
...
*/
ALTER TABLE DEPT_COPY
RENAME CONSTRAINT SYS_C008340 TO DEPT_ID_PK;
