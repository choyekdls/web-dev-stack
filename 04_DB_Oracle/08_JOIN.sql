...
-- RIGHT JOIN(오른쪽을 기준으로 잡을거냐)
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
RIGHT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);

-- FULL JOIN(왼, 오 둘다 기준이 됨 LEFT + RIGHT)
-- MySQL에서는 안씀
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
...
SELECT E.EMP_ID 사번, E.EMP_NAME 사원명, E.DEPT_CODE 사원부서코드, M.EMP_ID 사수사번, M.EMP_NAME 사수명, M.DEPT_CODE 사수부서코드
FROM EMPLOYEE E
...
SELECT LOCAOL_CODE, NATIONAL_CODE, LOCAL_NAME FROM LOCATION;
...
JOIN JOB USING (JOB_CODE)
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
WHERE EMP_NO LIKE '7%' AND EMP_NO LIKE '______-2%';
-- WHERE EMP_NO LIKE '7______-2% AND EMP_NAME LIKE '전%'

-- 3. 보너스를 받는 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역 조회 (단, 부서 배치 안된 사원들도 출력)//???????????????????왜 부서 배치 안된 사원 출력이 안되는거지???????????????????????????
...
SELECT DEPT_TITLE, TO_CHAR(AVG(SALARY), '999,999,999')
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
GROUP BY DEPT_TITLE;
...
-- 6. 사번, 직원명, 직급명, 급여 등급, 구분 조회 이때 구분은 급여 등급이 S1, S2인 경우 '고급', S3, S4인 경우 '중급', S5, S6인 경우 '초급' 
...
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
WHERE DEPT_CODE IS NOT NULL; -- 내부조인이라서 하나 안하나 똑같음 LEFT를 써야 부서가 없는 직원들의 정보도 볼 수 있음
...
SELECT * FROM REVIEW; -- MOVIE_ID
SELECT * FROM MOVIE; --MOVIE_ID
...
SELECT DISTINCT NAME -- 배우 이름 중복 제거
FROM MOVIE_ACTOR
JOIN MOVIE USING (MOVIE_ID)
JOIN ACTOR USING (ACTOR_ID)
WHERE DIRECTOR = '최동훈';

-- 15. 하정우가 출연한 영화 제목과 평균 평점 조회(대왕 복잡스리..)
SELECT * FROM REVIEW;
