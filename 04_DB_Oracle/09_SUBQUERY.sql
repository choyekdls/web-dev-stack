...
-- ㅅㅓ브쿼리 SELECT 절에서도 사용할 수 있다@-! (어렵네....)
SELECT EMP_ID, EMP_NAME, DEPT_CODE,
...
*/
-- 대리 직급임에도 불구하고 과장 직급들의 최소 급여보다 많이 받는 직원의 사번, 이름, 직급명, 급여조회
SELECT MIN(SALARY)
FROM EMPLOYEE
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
WHERE JOB_NAME = '대리' -- 2000000, 2550000, 2320000, 2780000, 3660000, 2436240 
...
FROM EMPLOYEE
WHERE EMP_NAME = '하이유') AND EMP_NAME != '하이유';
...
SELECT EMP_NAME,JOB_NAME, JOB_CODE
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);
...
FROM EMPLOYEE
WHERE (JOB_CODE, SALARY) IN (SELECT MIN(SALARY)
FROM EMPLOYEE
GROUP BY JOB_CODE);
...
-- WITH 사용 (TOPN 쿼리)
WITH TOPN_SAL AS (SELECT DEPT_CODE, ROUND(AVG(NVL(SALARY, 0))) AVG_SALARY
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
SELECT *
FROM (SELECT
DENSE_RANK() OVER(ORDER BY SALARY DESC) RANK, -- 별칭 제대로 써먹자 ㄱ-
EMP_NAME, SALARY
FROM EMPLOYEE)
WHERE RANK <= 19;
...
-- 2페이지 라면...? 11~20 숫자만 바꾸면 여러페이지 만들 수 있삼
SELECT *
FROM(
SELECT ROWNUM NUM,E.*
FROM(SELECT * FROM EMPLOYEE) E
...
-- 서울 사는 사용자 (12명 : 3,4,8,9,11,12,13,14,17,18,19,22)
...
WHERE USER_ID IN (SELECT DISTINCT USER_ID -- 값이 여러개인 경우 IN을 씁니다
FROM REVIEW
JOIN USER_INFO USING(USER_ID)
WHERE ADDRESS LIKE '서울%');
...
SELECT TITLE 
FROM (SELECT MOVIE_ID, AVG(RATING), 
DENSE_RANK()OVER(ORDER BY AVG(RATING)DESC) RANK
FROM REVIEW
JOIN MOVIE USING (MOVIE_ID)
GROUP BY TITLE
HAVING AVG(RAT==RATING) < (SELECT AVG(RATING) 
FROM REVIEW))
WHERE RANK = 3;
