/*
  ...
  START WITH 시작값 (지정안하면 기본 값 1부터 들어감)
  INCREMENT BY 증가값 (얼마씩 증가를 할거냐, 시본값은 1)
  MAXVALUE 최대값 (지정안하면 기본 값 : 기본값 큼)
  MINVALUE 최소값 (지정안하면 기본 값 :기본값 1)
...
*/
-- CURRVAL은 한번이라도 NEXTVAL이 수행하지 않으면 가져올 수 없음!
SELECT SEQ_EMPID.CURRVAL FROM DUAL;
-- MAXVALUE가 초과하고 NOCYCLE인 경우
SELECT SEQ_EMPID.NEXTVAL FROM DUAL; -- ORA-08004: 시퀀스 SEQ_EMPID.NEXTVAL exceeds MAXVALUE은 사례로 될 수 없습니다 발생
...
/*
  ...
  - DML(INSERT, UPDATE, DELETE) 작업 사능 (단, 일부만!)
...
/*
  ...
  CREATE OR REPLACE FORCE | NOFORCE VIES 뷰명
  AS 서브쿼리
  WITH CHECK OPTION
  WITH READ ONLY;
...
SELECT SYSDATE-EXTRACT(YEAR FROM HIRE_DATE) FROM EMPLOYEE;
SELECT CEIL((SYSDATE-HIRE_DATE)/365) FROM EMPLOYEE;
...
/*
  VIEW를 이용해서 DML(INSERTM UPDATE, DELETE) 사용 가능
...
-- INSERT
INSERT INTO VW_JOB VALUES('J8', '인턴'); -- 값의 수가 많아서 에러가 남!
...
UPDATE VW_JOB
SET JOB_NAME = '알바'
WHERE JOB_CODE = 'J8'; -- JOB_NAME이 VIEW에 존재하지 않아서 에러발생!
...
DELETE FROM VW_JOB
WHERE JOB_NAAME = '사원'; -- VIEW에 JOB_NAME이 존재하지 않아서 에러발생!

-- 2. 뷰에 포함되지 않은 컬럼 중에서 베이스가 되는 컬럼이 제약조건(NOT NULL)이 지정된 경우
CREATE OR REPLACE VIEW VW_JOB
AS SELECT JOB_NAME FROM JOB;

-- INSERT
INSERT INTO VW_JOB VALUES('인턴'); -- JOB_CODE가 NOT NULL 제약조건이 있어서 안됨
...
UPDATE VW_EMP_SAL
SET SALARY = 96000000/12
WHERE EMP_ID = 300;  -- 오예, 내 연봉 9600만원!
...
CREATE VIEW VW_GROUPDEPT
AS SELECT SUM(SALARY) 급여합계, AVG(SALARY) 급여평균, DEPT_CODE
FROM EMPLOYEE
GROUP BY DEPT_CODE;

-- INSERT
INSERT INTO VW_GROUPDEPT
VALUES("D0", 1000000, 100000); -- 그룹 함수들은 이미 계산된 함수식이기 때문에 쓸 수 없음
...
-- INSERT
-- 조인 뷰에 의하여 기본 테이블 이외의 컬럼을 수정하지 못한다 '총무부'는 추가할 수 없음
...
/*
  VIEW 옵션들 : FORCE | NOFORCE
  - FORCE: 존재하지 않아도 일단 뷰부터 생성 | --> 컴파일 오류와 함께 뷰가 생성되었습니다.
  - NOFORCE(기본값) : 기본 자체는 존재하는 테이블만 뷰가 생성됨
*/
CREATE OR REPLACE FORCE VIEW VW_EMP
AS SELECT * FROM TEST; --> 컴파일 오류와 함께 뷰가 생성되었습니다. 그래도 뷰는 만들어짐
...
CREATE SYNONYM EMP
FOR EMPLOYEE; -- EMPLOYEE에게 EMP라는 별칭을 ..주었다..

SELECT * FROM EMP;

-- 관리자 SYSTEM 계정으로 접속해서 공개SYNONYM 생성
...
-- 2. 명시적 커서(지대 복잡하고 지대 어렵고...난 울고싶고...)
...
/*
  ...
  CREATE OR REPLACE PROCEDURE 프로시저명(
    ...
    - OUT : PROCEDURE에서 로출 환경으로 값을 전달하는 역할
  )
  IS 선언부 (DECLARE 대신)
  BEGIN 실행부
  EXCEPTIOIN 예외처리부
...
CREATE OR REPLACE PROCEDUER DEL_ALL_EMP(
IS 
BEGIN 
...
END;
/
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME) VALUES(401, '뫄뫄');
...
AMOUNT NUMBER, -- 입출고수량
STATUS CHAR(10) CHECK(STATUS IN ('입고', '출고'))
);
 -- 1번 TV 상품 오늘날짜로 10개 입고
 INSERT INTO TB_PRODETAIL(PCODE, AMOUNT, STATUS)
 VALUES(1, 10, '입고');
 -- 해당 실행문이 실행됨과 동시에 1번 TV상품의 재고수량이 10개 증가 해야함
 UPDATE TB_PRODUCT
 SET STOCK = STOCK + 10
 WHERE PCODE = 1;
