/*
  ...
  * PL.SQL 구조
  - [선언부(DECLARE SECTION)] : DECLATE 시작, 변수나 상수를 선언 및 초기화
...
DBMS_OUTPUT.PUT_LINE('EID : ' || EID);
DBMS_OUTPUT.PUT_LINE('ENAME : ' || ENAME);
DBMS_OUTPUT.PUT_LINE('DEPT_TITLE : ' || EDEPT);
END;
/
...
/*
  실행부
  제어문 (조건문, 반복문, 쿼리문(SELECT)) 등의 로직을 기술
  
  1. IF문
  IF 조건식(괄호없음) THEN 실행내용 END IF;
*/
-- 사번을 입력받은 후 해당 사원의 사번, 이름, 보너스 출력
...
-- 보너스가 있는 경우만 보너스 출력, 없으면 해당 문구 출력
  ...
  DBMS_OUTPUT.PUT_LINE('이름은 ' || EMP.EMP_NAME);
  IF EMP.BONUS IS NULL THEN DBMS_OUTPUT.PUT_LINE('보너스를 지급받지 않는 사원입니다.');
  ELSIF DBMS_OUTPUT.PUT_LINE('보너스는 ' || EMP.BONUS);
  END IF;
  END;
  /
  /*
...
BEGIN 
  ...
  /*
   ...
   사원의 사번을 입력받아서 '해당 사원의 급여 등급은 S1입니다.'출력
  */
  DECLARE
  EID EMPLOYEE.EMP_ID%TYPE;
  ENAME EMPLOYEE.EMP_NAME%TYPE;
  ...
  ELSE GRADE := '초급';
  END IF;
  DBMS_OUTPUT.PUT_LINE(ENAME|| '의 급여 등급은 ' || GRADE || '입니다.');
  END;
  /
...
-- 그 외에는 서비스팀
-- 출력 :  해당 사원 이름 000은 개발팀입니다.

DECLARE 
EID EMPLOYEE.EMP_ID%TYPE;
ENAME EMPLOYEE.EMP_NAME%TYPE;
EDEPT EMPLOYEE.DEPT_CODE%TYPE;
ETITLE VARCHAR(30);
...
DBMS_OUTPUT.PUT_LINE(ENAME|| '은(는) ' || ETITLE || '입니다.');
END;
/
