/*
  LIMIT
  - ORDER BY 절보다 뒤에 조건을 걸고 싶을 때 사용 (ORDER BY 절 뒤에 위치함)
...
/*
  CONCAT(컬럼, 컬럼, ...)
  - 문자열을 하나로 합친 후 결과 반환 (검색 기능 - LIKE 랑 %)
*/
SELECT CONCAT(TITLE, DESCRIPTION, SPECIAL_FEATURES) FROM FILM;

/* 날짜 처리 함수 
   NOW(), CURRENT_TIMESTAMP() : 현재 날짜와 시간 반환 -- CURREBT_TIMESTAMP()는 추천하지 않습니다...
