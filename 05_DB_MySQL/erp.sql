...
CREATE TABLE COMPANY(
   COMPANY_NO INT AUTO_INCREMENT PRIMARY KEY,
   COMPANY_NAME VARCHAR(200) NOT NULL,
   ITEM_NO INT NOT NULL, -- 거래품목목
   ITEM_NAME VARCHAR(300) NOT NULL,
   FIRST_TRADE DATE NOT NULL,
   ADDR VARCHAR(500) NOT NULL,
   PHONE VARCHAR(50) NOT NULL,
   USER_NO INT -- 거래처별 본사 담당자
   );
   ALTER TABLE COMPANY ADD FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO); 
...
CREATE TABLE CLAIM (
   ...
   PHOTO -- 첨부파일? VARCHAR?,
   CLAIM_DESC TEXT,
   PRODUCT_RECALL VARCHAR(50) CHECK (RECALL IN ('전량회수', '일부회수', '회수불가'))
   EXPLANATION TEXT -- 클레임 관련 해명서
   COMPENSATION VARCHAR(500) NOT NULL
   COMPANY_NO INT,
   CUSTOMER_NO INT
);
ALTER TABLE CLAIM ADD FOREIGN KEY (COMPANY_NO) REFERENCES COMPANY(COMPANY_NO);
...
CREATE TABLE HR(
   ...
   NAME
   HIRE_DATE DATE
   LENGTH_OF DATE -- 근무년수에 따라 연차추가 지급 및 포상 지급급
   annual_leave_days INT DEFAULT 15, -- 연차 지급 수량은...연차 쌓일 수록....높아지고...근무기간 1년~3년 = 15개, 3년~5년 = 18개..이런식으로...
   used_annual_leave_days INT DEFAULT 0, 
   remaining_annual_leave AS (annual_leave_days - used_annual_leave_days) INT,
   EDUCATION VARCHAR(500) -- 교육, 훈련
   BENEFIT_NO INT AUTO_INCREMENT PRIMARY KEY, -- 복리후생
   BENEFIT_NAME VARCHAR(100)
);
ALTER TABLE HR ADD FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);
ALTER TABLE HR ADD FOREIGN KEY (NAME) REFERENCES USER_INFO(NAME);
ALTER TABLE HR ADD FOREIGN KEY (HIRE_DATE) REFERENCES USER_INFO(HIRE_DATE);
...
TRANS_DATE DATE NOT NULL, -- 지출 및 수입 내역 발생일자
CATEGORY VARCHAR(300) -- 지출발생 및 수입발생 카테고리
CREDIT_CARD INT, -- 법인카드 관리 (카드는 뒤 네자리번호만 .....)
USER_NO INT,
DEPT_NO INT,
COMPANY_NO INT,
COMPENSATION INT
...
ALTER TABLE FINANCIAL_TRANSACTION ADD FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);
ALTER TABLE FINANCIAL_TRANSACTION ADD FOREIGN KEY (DEPT_NO) REFERENCES DEPARTMENT(DEPT_NO);
ALTER TABLE FINANCIAL_TRANSACTION ADD FOREIGN KEY (COMPANY_NO) REFERENCES COMPANY(COMPANY_NO);
ALTER TABLE FINANCIAL_TRANSACTION ADD FOREIGN KEY (COMPENSATION) REFERENCES CUSTOMER(COMPENSATION);

-- 예산 관리
-- 부서별로 책정된 연간 예산
CREATE TABLE BUDGET (
   DEPT_NO INT,
   TARGET_SALES INT, -- 묙표매출
   ANNUAL_BUDGET INT NOT NULL,
   ANNUAL_PLAN TEXT, -- 각 부서별 예산 소진 계획
   TARGET_ACHIEVED BOOLEAN ('T', 'F') --목표매출 달성여부
);
ALTER TABLE FINANCIAL_TRANSACTION ADD FOREIGN KEY (USER_NO) REFERENCES USER_INFO(USER_NO);
...
CREATE TABLE STOCK(
   ...
   LOCATION VARCHAR(50) NOT NULL -- 제품 보관위치치
);
 ALTER TABLE STOCK ADD FOREIGN KEY (ITEM_NO) REFERENCES COMPANY(ITEM_NO); 
 ALTER TABLE STOCK ADD FOREIGN KEY (ITEM_NAME) REFERENCES COMPANY(ITEM_NAME); 

-- 품질검사 관리
CREATE TABLE QUALITY(
   QUALITY_TEST VARCHAR(50) CHECK (QUALITY_TEST IN ('진행', '미진행'))
   TEST_DATE DATE,
   TEST_RESULT VARCHAR(50) CHECJ (TEST_RESULT IN ('통과', '불량'))
);
...
ALTER TABLE DEFECT_ITEM ADD FOREIGN KEY (TEST_RESULT) REFERENCES QUALITY(TEST_RESULT); 
ALTER TABLE DEFECT_ITEM ADD FOREIGN KEY (ITEM_NO) REFERENCES COMPANY(ITEM_NO); 

-- 품질보증인증서
CREATE TABLE CERTIFICATE (
   CERT_NO INR AUTO_INCREMENT PRIMARY KEY,
