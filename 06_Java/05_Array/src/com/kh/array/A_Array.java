...
public class A_Array {
    ...
    */
    public void method3() {
    	// 아래 입력 내용을 for문으로 처리한다면?
    	double[] cmArr = new double[3];
    	double sum = 0;
    	for(int i = 0; i <cmArr.length; i++) {
    		System.out.print("키 입력 > ");
    		cmArr[i] = Double.parseDouble(sc.nextLine());
    		sum += cmArr[i];
    	}
    	double avg = sum/cmArr.length ;

    	for(int i = 0; i < cmArr.length; i++) {
    		System.out.println(cmArr[i]);
    	}
