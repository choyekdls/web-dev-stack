...
public class C_Sorting {
	...
	public void method1() {
		...
		// 2. 내림차순 정렬 (큰 숫자부터 작은 숫자) -> 원본 배열은 따로 있고 새로운 배열을 생성
		int[] copy = arr.clone();
		System.out.println(Arrays.toString(copy));
		// for(int i = 0; i < copy.length; i++) {
			// System.out.println(copy[i]);
		    // [copy.length - 1 - i] = arr[i]};
	...
	/*
		 ...
		 * 실제로는 Arrays.sort() 같은 내장 정렬이나 Collections.sort()가 훨씬 빠르고 실용적이기 때문에 몰라도 된다고 한다..!
		 * */
