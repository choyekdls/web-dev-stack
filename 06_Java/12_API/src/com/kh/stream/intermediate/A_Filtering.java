...
public class A_Filtering {
	...
	List<Student> students = Arrays.asList(
			...
			new Student("이환히", 19, "여자", 50, 100), 
			new Student("박기민", 19, "남자", 70, 90),
			new Student("조규상", 19, "남자", 75, 85), 
			new Student("이상엽", 19, "남자", 100, 60),
	...
	// 중간처리메서드의 특징 : 중복하여 사용가능 (최종처리 전에만 사용하면 됩니다..)
	public void method2() {
		names.stream()
		.distinct()
		.filter(name -> name.startsWith("이")).forEach(name -> System.out.println(name));
		...
		.filter(student -> student.getEng() > 70)
		// .filter(student -> student.getMath() > 70 && student.getEng() > 70)
		.distinct()
		.forEach(student -> System.out.println(student));
