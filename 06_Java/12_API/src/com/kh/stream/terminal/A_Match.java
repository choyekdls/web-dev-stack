 ...
 * - allMacth : 모든 요소들이 매개 값으로 주어진 조건을 만족하는지 조사
 * - anyMatch : 최소한 한 개의 요소가 매개 값으로 주어진 조건을 만족하는지 조사
 * - noneMatch : 모든 요소들이 매개 값으로 주어진 조건을 만족하지 않는지 조사 (allMatch의 반대입니다..)
 * */
...
public class A_Match {
	...
	public void method1() {
		...
		System.out.println("모든 수가 다 짝수인가? " + result);
	...
	public void method2() {
		...
		boolean result = 
				students
				.stream()
				.allMatch(student -> student.getGender().equals("남자") && student.getAge() <= 18);
		System.out.println("나이가 18살 이하인 학생들은 모두 남자인가? " + result);
		...
		boolean result2 = 
				...
				.anyMatch((student -> (student.getMath() + student.getEng()) / 2 >= 90));
				System.out.println("남학생들 중에 평균이 90점 이상인 학생이 한명이라도 존재하는가? " + result2);
