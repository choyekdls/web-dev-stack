...
public class C_Collect {
	...
	public static void main(String[] args) {
		List<Student> students = Arrays.asList(new Student("오재덕", 20, "남자", 80, 90),
				new Student("이승민", 19, "남자", 75, 80), new Student("이환희", 19, "여자", 50, 100),
				new Student("박기민", 19, "남자", 70, 90), new Student("조규상", 19, "남자", 75, 85),
				new Student("이상엽", 19, "남자", 100, 60), new Student("성예찬", 19, "남자", 70, 95));

		// List 컬렉션으로 수집 -> 영어 점수가 90점 이상인 사람들만
		List<Student> list = students.stream().filter(student -> student.getEng() >= 90)
				// .collect(Collectors.toList()); //조건에 맞은 항목들을 collect 해서 list 타입으로 가져옵니다..
		...
		HashSet<Student> set1 = students.stream()
				.filter(student -> student.getAge() < 20)
