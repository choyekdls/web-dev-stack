...
function btn1() {
  ...
  console.log(divs);
  console.log(divs[0]); //첫 번째 div 소환 : 배열 소환하는 거랑 똑같음
}
...
function btn3() {
  ...
  console.log(div);
  console.log(div[1]); // 두 번째 Class 소환
}
...
// CSS 선택자 문법 사용 가능
function btn5() {
  //한 개만 가지고 오고 싶다면 querySelectot
  const div = document.querySelector("#testId");
  console.log(div);

  // 여러개를 가지고 오고 싶다면 querySelelctorAll
...
function btn7() {
  const div = document.querySelector("#testId");
  // 속성 추가 & 수정
  div.setAttribute("data-test", "테스트"); //앞에는 속성명, 뒤에는 속성 값
  // 속성 값 가져오기
...
function btn11() {
  const has = div2.classList.contains("black"); // black 클래스가 있는지 확인 (true/false로 나타냄)
...
function btn12() {
  div2.classList.toggle("black"); // 알아서 있으면 없애고, 없으면 만들고
}
