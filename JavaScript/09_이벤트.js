...
window.addEventListener("DOMContentLoaded", () => {
  ...
  hover.addEventListener("mouseenter", () => {
    ...
    // 배경색상 변경, 텍스트는 Mouse Enter! 변경
    hover.style.background = "hotpink";
    hover.textContent = "Mouse Enter!";
  });
  hover.addEventListener("mouseleave", () => {
    console.log("mouseleave");
    // 배경색상 변경, 텍스트는 Mouse Leave! 변경
  ...
  window.addEventListener("wheel", (e) => {
    ...
    else if (e.deltaY < 0) wheel.textContent = "휠 아래로 스크린 중!";
  });
  window.addEventListener("scroll", () => {
    ...
    // scrollHeoghr - innerHeight : 스크롤 총 내리는 높이
    const totalHeight =
  ...
  const removeHandler = (e) => {
    console.log("target", e.target); // 내가 클릭하는 이미지가 선택됨
    console.log("current", e.currentTarget); // 이벤트 걸린 본인 선택
